...
func (u *TelegramUsecase) ConnectTelegram(ctx context.Context, karyawanID string, verificationCode string) error {
	...
	if strings.HasPrefix(verificationCode, "SAKTI-") {
		hexPart := verificationCode[6:]
		verificationCode = "SAKTI-" + strings.ToLower(hexPart)
	} else if strings.HasPrefix(strings.ToLower(verificationCode), "sakti-") {
		hexPart := verificationCode[6:]
		verificationCode = "SAKTI-" + strings.ToLower(hexPart)
	}
