...
func (r *LiburRepo) GetAll(ctx context.Context, tahun int, jenis, sumber string, aktif *bool, limit, offset int) ([]domain.Libur, int, error) {
	...
	if tahun > 0 {
		baseQuery += ` AND EXTRACT(YEAR FROM tanggal) = $` + string(rune(argIdx+'0'))
		args = append(args, tahun)
		argIdx++
	}
	if jenis != "" {
		baseQuery += ` AND jenis = $` + string(rune(argIdx+'0'))
		args = append(args, jenis)
		argIdx++
	}
	if sumber != "" {
		baseQuery += ` AND sumber = $` + string(rune(argIdx+'0'))
		args = append(args, sumber)
		argIdx++
	}
	if aktif != nil {
		baseQuery += ` AND aktif = $` + string(rune(argIdx+'0'))
	...
	dataQuery := `
		SELECT id, tanggal, nama, jenis, aktif, sumber, dibuat_pada, diperbarui_pada
	` + baseQuery + ` ORDER BY tanggal ASC LIMIT $` + string(rune(argIdx+'0')) + ` OFFSET $` + string(rune(argIdx+1+'0'))
...
func (r *LiburRepo) Update(ctx context.Context, libur *domain.Libur) error {
	query := `
		UPDATE libur 
		SET nama = $2, jenis = $3, diperbarui_pada = NOW()
		WHERE id = $1
	`
	_, err := r.DB.Exec(ctx, query, libur.ID, libur.Nama, libur.Jenis)
	return err
}
