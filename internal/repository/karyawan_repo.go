...
func (r *KaryawanRepo) GetAllByRole(ctx context.Context, role string) ([]domain.Karyawan, error) {
	query := `
		SELECT id, nama_lengkap, email, nomor_telepon, foto_url, 
			   role, level_jabatan, atasan_langsung_id, 
			   divisi, unit, status_karyawan, telegram_chat_id, dibuat_pada, diperbarui_pada
...
func (r *KaryawanRepo) GetAtasanByKaryawanID(ctx context.Context, karyawanID string) (*domain.Karyawan, error) {
	query := `
		SELECT k.id, k.nama_lengkap, k.email, k.nomor_telepon, k.foto_url, 
			   k.role, k.level_jabatan, k.atasan_langsung_id, 
			   k.divisi, k.unit, k.status_karyawan, k.telegram_chat_id, k.dibuat_pada, k.diperbarui_pada
		FROM karyawan k
		JOIN karyawan k2 ON k2.atasan_langsung_id = k.id
		WHERE k2.id = $1 AND k.status_karyawan = 'aktif'
		LIMIT 1
	`
	...
	err := r.DB.QueryRow(ctx, query, karyawanID).Scan(
		...
		&atasan.Role,
		&atasan.LevelJabatan,
		&atasan.AtasanLangsungID,
		&atasan.Divisi,
		&atasan.Unit,
		&atasan.StatusKaryawan,
		&atasan.TelegramChatID,
		&atasan.DibuatPada,
		&atasan.DiperbaruiPada,
	)
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			return nil, nil
	...
	atasan.Departemen = getDepartemen(atasan.Divisi, atasan.Unit)
	return &atasan, nil
}
