...
func main() {
    ...
    app.Use(cors.New(cors.Config{
        AllowOrigins: "https://adminsakti.kopegtelmalang.co.id,http://127.0.0.1:5173,http://localhost:5173",
    ...
    go func() {
        for {
            now := time.Now()
            next := time.Date(now.Year()+1, 1, 1, 0, 0, 0, 0, time.Local)
            time.Sleep(next.Sub(now))

            ctx := context.Background()
            if err := leaveUsecase.GenerateAnnualLeaveForAll(ctx, next.Year()); err != nil {
                log.Printf("Generate annual leave failed: %v", err)
            }
        }
    }()
